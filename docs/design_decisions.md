...
## DD-053: the rule-conformance review — a rule that passes its controls has not been shown to measure what the spec says

**Date:** 2026-09-06. **Task:** `cc_tasks/2026-09-06_scan_targets.md` §2. **Instrument:** `scripts/rule_review.py`, adversarial-review baseline rubric **v1.3.0**, project-local overlay `rule-conformance`. **Result:** of 16 `v1` rules, **0 conforming**, 13 `deviates`, 3 `spec_underspecified`.

**1. The gap this closes, in the harness's own words.** `2026-09-06_harness_scaffold_RESULT.md` §9: *"Every rule is at `v1` and none has been reviewed against its `MeasurementSpec` by anyone but its author."* The control fixtures prove a rule is **self-consistent** — it says `pass` on a good surface and `fail` on a bad one. They cannot prove it is **the measurement the framework specified**, because the fixtures were built by the same author in the same sitting as the rules, from the same reading of the spec. A shared misreading passes a control gate unanimously. Conformance is a different question and needs a different instrument.

**2. It is a review, not a rating, so the output is a diff and there is no second rater.** Inter-rater agreement is worth measuring when the target is a judgement. "Does line 24 implement the sentence *HEAD and read Content-Type*" is not a judgement, it is a reading, and a κ between two models reading the same sentence would measure their shared idiom rather than the code. No Fable pass, no kappa, and the calibration is §2's own **verification of every finding against the collector** (§3 below) rather than an inherited agreement rate.

**3. Anti-anchoring: the smoke-run verdicts were withheld.** Rubric v1.3.0 §2 — never show the judge the number the item was selected on. A reviewer told *"A2 failed on 15 of 15 surfaces"* reasons backward from the outcome: a rule that fails everything looks broken, a rule that passes looks right, and neither is evidence about conformance. This is the same discipline that kept the cosine out of `link_judge.py`'s prompt, and the reason it matters here is that the smoke run's verdicts are dominated by the surfaces, not by the rules.

**4. The result is 0 of 16 conforming, and the shape is consistent.** In twelve of thirteen `deviates` the rule implements the FIRST clause of a multi-clause `signal` and stops: A2's spec says *record auth scheme, declared rate limits, and whether the description is machine-readable* and the rule checked only machine-readability; A6's says *validate against DCAT-AP SHACL shapes* with `pyshacl` in its own `collector_pin`, and the rule matched a type name; D4's says *validate against the POD v1.1 schema* and no branch consumed a validation result; A8's says *test whether a latest-vintage pointer resolves* and nothing was ever dereferenced. This is not sixteen unrelated bugs. It is one habit — **build the cheapest testable clause, and let the spec's remaining clauses stand as intent** — and it is invisible from inside a control gate because the fixture was built to the same cheap clause.

**5. A model finding is a finding to VERIFY, not a verdict to obey.** Every `deviates` was checked by hand against the collector and the runner before any `v2` was written, and **one did not survive**: G1-D's reviewer said no test distinguishes a structured field from footnote prose, and the field-scoping is real — it lives in `runner.py`, which slices `<th>` cells, a CSV header row, or a JSON head before the rule ever sees a token. **The review's information boundary caused it**: §2 of the task specified *the rule module source*, and a rule is half of a measurement. Recorded as a limitation of this protocol: **a conformance review that sees only the rule will over-report deviation whenever a clause is satisfied upstream.** The next run of this instrument should ship the collector path with the rule.

**6. `deviates` gets a `v2` module; `spec_underspecified` gets a `decision` on the spec.** Twelve `v2` modules were written and **not one `v1` line was edited**: a stored Finding must re-derive under the rule that made it, so `rules/__init__` now carries `REGISTRY` (every version ever shipped) beside `CURRENT` (what a new cycle judges with), and `rederive.py` re-judges each Finding under **its own** `rule_id`. The three `spec_underspecified` legs — A4 (is a missing robots.txt a pass or a fail?), A5 (does a served llms.txt satisfy discoverability?), A9 (what counts as a served machine-first entry point?) — got no code change at all. Their rules already decided; the decision was simply not written down, and writing it into the `MeasurementSpec` closes the gap **in the framework of record** rather than in a Python module where the next reader would have to find it by reading code.

**7. What the controls could not have caught, stated plainly.** In all three underspecified cases both fixtures land on their stated verdicts under either reading. The disagreement is only visible on a real surface. That is the precise sense in which a green control gate is necessary and not sufficient, and it is why this review had to be a separate instrument rather than another fixture.


## DD-054: A12 is a CANDIDATE indicator — the framework does not adopt what the instrument found about itself without the operator

**Date:** 2026-09-06. **Task:** `cc_tasks/2026-09-06_scan_targets.md` §4. **Status:** `candidate`, not counted in any criterion, not in any progress fraction, rendered in its own table.

**The indicator.** *An identified, robots-compliant machine client that robots.txt permits is served — not refused by a WAF or bot manager.*

**1. It is the public-observable leg of what A11 assumed needed edge logs.** A11 splits crawler access into declared / enforced / observed and puts the last two at `agency_instrumented`, on the reasoning that you cannot see an agency's WAF from outside. That is true of the enforced layer **in general** and false of one measurable corner of it: when robots.txt PERMITS a path and the host answers 403 to a compliant client asking for **that same path**, the two layers disagree, and the disagreement is visible from the public tier with no logs at all. A12 measures only that disagreement. Where robots.txt disallows, A12 is `not_applicable` — that case is A4's measurement, not a coherence failure.

**2. It was found by the harness, not by the literature, and that changes its standing.** The scaffold's smoke run recorded `www.bls.gov` refusing 60 of 60 requests; this task's pre-flight found the same at `www.bts.gov` and `www.ssa.gov`. **Three of the thirteen U.S. principal statistical agencies** refuse an identified, rate-limited, robots-obeying client while their own robots.txt permits the paths. No source in the corpus proposes this as an AI-readiness construct — every other indicator in the framework was crosswalked out of a published framework or a named standard, and this one was produced by the instrument measuring the world and noticing something the literature has not named. That is a legitimate way to find an indicator and an illegitimate way to adopt one silently.

**3. Promotion is an operator decision, and separation is the mechanism.** The framework goes out under the operator's name. A candidate therefore renders into a **separate table** rather than into criterion A with a status column: a reader scanning the A table must not have to check a column to know whether a row is part of the instrument, and a status column is exactly the kind of distinction that survives in the JSON and dies in the reading. `rows_from_json` excludes candidates, `skeleton_rows` truncates at the candidate heading so the row cannot leak back in through the round trip, and two tests hold both.
...
## DD-058: evidence enters the committed store on citation, and bytes that record contact with a public host are retained even when nothing cites them

**Date:** 2026-09-07. **Task:** `cc_tasks/2026-09-07_scan_run_2.md` §1.1, §1.4. **Enforced by:** `assessment/harness/scan/publish.py::promote_evidence`, `assessment/harness/scan/run.py --evidence-root`, `tests/test_scan_run_2.py`.

**The defect.** `corpus/evidence/scan/` is the one `corpus/` lane that is tracked, because it holds the bytes a Finding cites — a verdict whose evidence the repo does not hold is not evidence (invariant 3). But every scan run wrote every body it fetched straight into it, published or not. Content addressing made that cheap to do and impossible to sort out afterwards: a digest names the bytes and says nothing about who wanted them. Two measured consequences. **260** fixture blobs from `tests/test_scan_harness.py` were committed before `tests/conftest.py`'s guard existed — the fixture server binds an ephemeral port that lands in every body carrying `HOSTPORT`, so content addressing minted a fresh digest per run instead of deduping. And **418** bodies from real federal hosts sit in the store cited by no Observation on the log: diagnostic runs, aborted runs, and the preflight.

**The rule, one direction.** A cycle STAGES into `state/evidence_staging/<cycle.name>/` (gitignored). `publish.promote_evidence` copies into the committed store exactly the digests the published payload's Observations cite, rewrites each `body_path` to the promoted location before any event is written, and deletes the staging directory. A cited digest present in neither place is a hard failure. This is invariant 3 applied one layer down: *no citation, no committed byte*. An unpublished run now leaves nothing behind, which is the property `--evidence-root` exists for and which no guard outside pytest previously had.

Rewriting `body_path` is safe and is the point: it is not an input to the derived `obs_id` (`model.Observation.make` hashes `body_sha256`), so the record keeps its identity, and a staging path on an append-only line would be a dangling reference to a directory the same run deletes.

**The 418 are retained where they are.** Not promoted, not deleted, not cited. The reasoning is a split the fixture sweep already made and this decision names: `scripts/quarantine_fixture_evidence.py` REMOVED untracked fixture litter and MOVED the tracked kind to `corpus/quarantine/`, because invariant 2 protects acquisitions and a fixture body is not one. A body fetched from `www.bls.gov` **is** one. It is the record that this scanner contacted that host on that day, under that UA, and got those bytes — the auditable half of the manners claim the harness makes in public (RFC 9309 compliance, 1 req/s, identified client). Deleting it to tidy a count would destroy the only evidence for a claim about our own conduct toward someone else's server, which is the wrong side of invariant 2 whatever the citation status.

So they are: **registered** as a count (`scan_evidence_uncited_real_host_bodies`), **excluded** from every integrity check that asks "is each committed body cited" — the check is over bodies promoted under this rule, not over the ones that predate it — and **never cited by anything**. A future task that wants them gone needs a retention policy for contact records, which is a different question from evidence hygiene and should not be decided by a cleanup script.

**What this does not do.** It does not retroactively promote or re-stage the four cycles published before staging existed; their bodies are already committed and `publish.py --no-promote` is how those payloads are re-published. It does not make the count zero and it is not supposed to: 418 is a fact about four days of instrument development, and a number that a decision explains is worth more than a number a script erased.

**Amendment (2026-09-07, same day, before this task's RESULT was written; recorded per the constitution's "amendments self-execute with grounding on their face and a diff in the log").** One sentence above was falsified by executing the decision it records.

"Not re-measurable by a later cycle: staging means a cycle promotes only what it cites" is right about the direction that matters and wrong about the other one. Staging forbids the set **growing** — no later cycle can add an uncited body, which is the whole point. It does not forbid the set **shrinking**, and content addressing is why: a cycle that re-fetches an unchanged page produces a digest the store already holds, and if that body was one of the uncited ones it becomes cited without a byte moving. Cycle 2 did this exactly once — `bjs.ojp.gov/data/topic` came back byte-identical to a body left by an earlier diagnostic run — so the census went **418 → 417**.
...
**The log records every judgement the project asserts, not only every Observation. A cycle's shard is named from the cycle. Supersession is an edge, not a convention.**

*2026-09-14. `docs/design/2026-09-14_DN-003_event_log_and_rejudgements.md` decisions 1 to 6, executed by `cc_tasks/2026-09-14_rejudgements_on_the_log.md`. Under DD-001, DD-008, DD-019, DD-055.*

**The condition this decides.** Fourteen re-judged payloads existed in `state/`; two were on the log and twelve were not, including `scan_2026-09-10_rj2`, the judgement of record for the published report. The graph therefore held, for cycle 4, the measured judgement that cycle's own gate had refused, while the report quoted a judgement that appeared nowhere in the append-only record. A stranger replaying the log got a different instrument than the one that published, which is what DD-001 forbids.

**1. A re-judgement is a judgement and goes on the log; it creates no evidence and promotes none.** Its `finding_derived` events cite the source cycle's `obs_id`s and no Observation is written. `publish.write_events` refuses a re-judged payload that carries an Observation, one that cites an `obs_id` not *already* on the log, and one whose predecessor is not on the log — three refusals saying the same thing from three sides. The second is stricter than the rule for a measured cycle, which may publish a Finding and its evidence in one breath: a judgement whose whole claim is that the evidence was already sufficient has no such licence.

**2. One shard per cycle, named from the cycle — `events/cycle-<name>.jsonl`.** DD-008 shards by ingest *batch*, which is the right unit for an extraction run and the wrong one for an assessment cycle: a cycle already has a name that `state/`, the matrices, the figures and the report's `snapshot_cycle` all key on. The typed `publish.CYCLE_BATCH` table this replaces had to be edited before every publication and sent an unlisted cycle to a shard three other cycles owned. `shard_for` now *discovers* the shard a cycle's events are already on and falls back to the named one, so "one shard per cycle" and "the log is never rewritten" hold together: `scan_2026-09-07_rj1` and `scan_2026-09-07b_rj1` keep `batch-040` and `batch-041`.

Cycle shards are **graph** shards: `eventlog.replay()` yields them by default, after the numbered batches and in name order. They are not tagged shards, which are off-graph by construction. Because shard order no longer guarantees that a cited Observation was written before the Finding citing it, `publish.project` buffers Finding events and writes them after the Observations of the same replay — the projection is now independent of shard order rather than relying on batch numbers happening to increase.

**3. Supersession is an edge on the graph, projected from an event.** A `finding_supersedes` event pairs each re-judged Finding with the one it replaces on the same (site, leg), one to one; the projection MERGEs `(new)-[:SUPERSEDES]->(old)` and sets `Finding.current` where nothing points at it. The pairing is computed once at publish time from the two payloads and never recomputed — the projection compares no verdict and takes no diff. The alternative, recomputing the chain from `state/` at projection time, is exactly the convention this replaces: the graph would be answering from a directory beside the log rather than from the log.

**Predecessor is the most recent judgement of the same cycle**, derived from the name (`_rjN` replaces `_rj(N-1)`, `_rj1` replaces the measurement), which is the doctrine `scripts/rejudge_gen9.py` and `scripts/rejudge_gen10.py` already state. It is *not* read from the `rejudgement_diff` records: the 2026-09-10 record pairs `scan_2026-09-07b_rj2` with the measured cycle although `_rj1` was already published, because that task's question was a diff and not a chain.

**4. Publication is in generation order**, derived by sorting on `(generation, cycle)` rather than typed, so no judgement can be published before the one it replaces — and the log's sequence is the sequence in which the project asserted its judgements.

**5. From now on a re-judgement task publishes its payloads as its last step.** A re-judged payload in `state/` and not on the log is a gate failure for the task that made it.

*Prior art, and it is ordinary.* This is event sourcing's standing answer to a corrected read model: the correction is a new event, never an edit, and the projection is rebuilt from the log — the same shape this repo already uses for `extraction_superseded` (batch-004), `edge_endpoint_alias` (batch-005) and `finding_evidence_unretained` (batch-032). Bitemporality is the other half of the name: an Observation carries when the world was observed, a Finding carries when the project asserted a judgement about it, and `SUPERSEDES` is what makes "as of which assertion" answerable without mutating either.

---

## DD-066

**G1-D is a product-tier indicator; the host-level leg is withdrawn for cycle 5 forward, and history stays as measured.**

*Date:* 2026-09-15. *Task:* `cc_tasks/2026-09-15_g1d_leaves_l0.md` and its ADDENDUM_01. *Under:* DD-001, DD-052 §3 and §6, DD-055, DD-059, DD-064 §2, DD-065, DN-004, DN-005 §2.1 and §2.2. *Value input:* the operator's, 2026-09-15.

**1. The construct is right and the level was wrong.** G1-D asks whether a published figure carries, beside it and in machine-readable form, the fields that make it interpretable: a measure of uncertainty, a standard error or CV, a suppression flag, a reliability marker. The operator's restatement of 2026-09-15 is adopted as the construct's definition: **uncertainty present for the human reader and absent from the markup.** That is a property of a data product. At host level the surface is a home page, which carries no estimate, so the check cannot hold the property it measures and every verdict on it is structural rather than a measurement of the publisher.

**2. The evidence is the census, not the argument.** Across cycles 1 to 4 and the self cycle, over every body and every reference host, the host-level leg returned **126 `fail`, 28 `error` and not one `pass`** (154 Findings). The same rule, in the same runs, returned **104 `pass`** on product surfaces. A check that never passes on one surface kind and passes 104 times on another is measuring which surface it was pointed at. The Cypher and the full per-cycle, per-tier breakdown are in that task's RESULT §0; the count is regenerated on every report build by `scripts/withdrawn_legs.py`, which **refuses the build if a host-level `pass` ever appears** — the withdrawal's premise, re-checked rather than asserted once.

**3. The instrument changes; the history does not.** `G1-D` leaves `params.tier0.legs` and is no longer dispatched on `home` or `well_known` surfaces (`params.tier0.legs_withdrawn`, read by `run.withdrawn_on`). It keeps its rule module, its registry entry and its dispatch on **product** surfaces. Cycles 1 to 4 and the self cycle keep every G1-D Observation and Finding on the append-only log under their own `params_hash` and `harness_version` (DD-052 §3, DD-064 §2). **No re-judgement is commissioned.** A re-judgement to `not_applicable` would assert the leg was measurable all along and mis-scored; it was not measurable on that surface, and the record should say so by ceasing to measure it, not by rewriting a verdict that was correct for the question asked.

**4. Removing the leg from `tier0.legs` was not enough, and that is worth recording.** A tier-A `home` surface never took its legs from that list: `run.targets` gave `home` and `flagship` alike the framework set derived from the rule REGISTRY (`CONTROL_LEGS`), and only Tier C read `tier0.legs`. A withdrawal expressed only in the tier-0 list would have gone on judging G1-D on all sixteen agencies' home pages while the published matrix no longer showed the column. The withdrawal is therefore **surface-kind scoped** and declared in `params.yaml` with its construct, its reason and this DD beside it.

**5. Five published Results are withdrawn, none deleted, and two of them for a second-order reason.** The three the report quoted at host level — the pass count, its denominator and the Wilson upper bound — move to `stale` with the reason on the artifact. **There is no `withdrawn` state on the Result state machine** (`proposed → [verified, rejected]`, `verified → [published, stale]`, `published → [stale]`, `stale → [verified]`); `stale` is the reachable state and is the state machine's own word for a value that stands as measured and is no longer the current instrument's answer. Two further published Results — the home/flagship disagreement cell and body counts — were computed over the *five* surface-judged tier-0 legs that included G1-D and are now a four-leg comparison; a name binds once (AD-028), so they are withdrawn at their true five-leg values and the new comparison is registered as `scan_l0_home_flagship_disagreement_{cells,bodies}_over_four_legs_<cycle>`. Every other published Result keeps its value and state, and the report's snapshot stays `scan_2026-09-10_rj2`: no Observation, Finding or stored payload moved.

**6. The framework record tags it.** `ind:G1-D` carries `measurement_tier: product`, `construct_restated` and `measurement_tier_source`, written through `scripts/framework_writeback.py::save` (the single writer) and projected. This is the **first** `measurement_tier` value in the record; the capability-tiering task (DN-005 §4 item 2) fills in the rest and may rename the field, but it does not remove this one.

**7. Prior art, named and explicitly not yet evaluated.** A product-tier G1-D rule should test against a standard rather than a token list. The standards it would bind to are **SDMX** observation-status and confidence attributes, the **W3C Data Quality Vocabulary (DQV)**, and **schema.org `Dataset`** variable metadata. **None of the three has yet been evaluated against a federal data product by this project**, and nothing here claims otherwise; the token-match rule that shipped (`RULE-G1-D-v1`, DD-036's frozen `g1_declared` probe as instrument of record) is what exists today. Naming them is the search the next builder does not have to repeat; evaluating them is the January G1 pilot's work (`73f0aa5d`, instrument frozen at v2).

**8. What this does not do.** It does not define the levels above L0, does not score anything, and does not tier any other indicator. It removes one column that measured the surface it was pointed at instead of the publisher, and it says where that column went.

---

## DD-067

**A re-judgement may re-read an Observation's retained body with the collector's own pure parsers, and it judges each surface on the frame's legs only. Both are recorded on the payload and honoured by the re-derivation gate.**

*Date:* 2026-09-18. *Task:* `cc_tasks/2026-09-18_rejudge_seven_legs.md`. *Under:* DD-041, DD-052, DD-059, DD-065, DD-066, DN-003 decision 2.

**1. The situation.** Generations 11 to 13 added three parsed blocks that the collector computes, at collection time, from a body it has already fetched: D4's `dcat_fields` and `membership`, and A4's `content_signal`. Every stored Observation predates all three. The rules that read them (`RULE-B1-v2`, `B4-v1`, `D3-v1`, `G4-v1`, `D2-v1`, `D4-v3`) therefore return `error`, "stored before the block existed", over evidence whose bytes are retained, content-addressed, under `corpus/evidence/scan/`, and whose sha256 is on the Observation on the log.

**2. The decision: re-read, do not re-measure.** `scan/reread.py` fills an absent block from the retained body, calling the same functions with the same arguments the collector calls (`dcat.membership_block`, `v2clauses.dcat_record_fields`, `v2clauses.content_signals`). It checks the body's sha256 against the Observation first and refuses on a mismatch or a missing file. It never overwrites a block that was recorded, and it never mutates the stored row. `obs_id` is derived from the leg, target, collector, params and body digest, never from `parsed` (`model.Observation.make`), so every Finding cites exactly the `obs_id`s already on the log. No byte is fetched and no Observation is created. That is DN-003 decision 2's line between a judgement and a measurement, and this stays on the judgement side of it. The payload records what was re-read as `observations_reread`, and `rederive.observations_for` applies the same re-read to a payload that carries the record. The re-derivation gate therefore judges exactly what the re-judgement judged (DD-041). A payload without the record is re-derived exactly as before.

*Prior art.* This is re-analysis of retained raw data under a corrected reader, the licence `rederive.py`'s re-judgement section already cites. It is also DD-052's split: Observations are evidence, and anything a pure function computes from their retained bytes is derivable, not observed. The rejected alternative was to publish the six rules as `error` everywhere until cycle 5. That would have published a scope limitation that does not exist: the evidence is on disk, and `error` would claim the collector could not see it.

**3. The frame decides which legs a surface carries, in a re-judgement as in a cycle.** `rederive.rejudge` used to judge every current leg wherever its evidence existed. A rule that reads a *shared* leg therefore landed wherever that leg was collected. `RULE-D2-v1` reads A4, A4 is collected on the three Tier C reference hosts, and DD-059 keeps every leg but `tier0.legs` off them. So the re-judgement now takes `frame=run.targets(params)` and judges each surface on that row's legs only, as `run.run_surface` does. The map goes on the payload as `surface_legs`, and `rederive.rederive` holds itself to the recorded map. `run.py` now records `surface_legs` on every measured payload as well. Without it, cycle 5's own re-derivation gate would have re-derived D2 on the Tier C rows and reported Findings the cycle never recorded as `unexpected_after_rederive`.
