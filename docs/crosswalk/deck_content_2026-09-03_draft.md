# Framework Deck — slide content v6 (DRAFT: adds the G1 dissociation slide)

**Date:** 2026-09-03 (v6 DRAFT: one slide added after the G1 slide — the declared→observed dissociation, from the v2 EVAL; slides 14–18 of v5 renumbered 15–19; no other slide changed — task `cc_tasks/2026-09-03_g1_freeze_calibration_redefinition_findings.md` step 4. The new slide's numbers are `{{result:<NAME>:value}}` tokens resolved at build time by `scripts/g1_resolve_results.py` against registered Seldon Results; every other slide's numbers are unchanged literals from v5. This deck is a DRAFT and goes nowhere without the operator.) 2026-09-02 (v5: slide 15 admitted count 194 → 211 after the G1-EVAL prior-art admission of 2026-09-02, extracted stated as 35 of 211; every slide-15 number now resolves to a registered Seldon Result — task `cc_tasks/2026-09-02_post_burn_reconciliation.md`; no other slide changed). 2026-09-02 (v4: slide 15 floors replaced with measured post-burn counts and the acceptance-sampling sentence corrected — task `cc_tasks/2026-09-02_deck_numbers_post_burn.md`; no other slide changed). 2026-09-01 (v3: orientation-first reframe of slide 5, discovery surfaces on 8, reference implementation on 9, frontier-candidate rule on 13, June work as absorbed prior art on 16 — task `cc_tasks/2026-09-01_assessment_consolidation.md`. v2: added slides 2, 4, 8, 14 — why-now, OV-1, SV-1, knowledge architecture). Desktop-drafted content for the hybrid AI Data Readiness framework deck.
**Build:** `cc_tasks/2026-09-01_framework_deck_build.md` (python-pptx, plain text, no theme).
**Numbers:** measured 2026-09-02 after the v038 burn closed — derivation table in `cc_tasks/2026-09-02_deck_numbers_post_burn_RESULT.md` (live graph, event log, queue, burn state file), admitted count re-measured after batch-025 and every slide-15 value registered as a Seldon Result artifact by `cc_tasks/2026-09-02_post_burn_reconciliation_RESULT.md`. Resolve to those Results (`seldon result list`) before any external presentation.
...
## Slide 14 — Structured fields are necessary for the declared leg and not sufficient for the observed one

- G1 is measured on two legs on the *same* captured surface file. **Declared:** are the error measures there as structured fields? **Observed:** does an AI consumer restating that surface carry them? The two legs dissociate — the surfaces that score best on the first score worst on the second.
- Family loss when the consumer is asked for a summary, by surface type, at no compression budget (pooled, pinned consumer `claude-opus-5`, parser `g1-parse-v2`, scorer `g1-score-v2`):

    surface type     declared leg   observed loss   scored families
    table_coded      PASS           {{result:g1_v2_surface_table_coded_none_loss_rate:value}}          {{result:g1_v2_surface_table_coded_none_scored:value}}
    table_labeled    PARTIAL        {{result:g1_v2_surface_table_labeled_none_loss_rate:value}}         {{result:g1_v2_surface_table_labeled_none_scored:value}}
    flagged_cell     PARTIAL        {{result:g1_v2_surface_flagged_cell_none_loss_rate:value}}         {{result:g1_v2_surface_flagged_cell_none_scored:value}}
    footnoted        PARTIAL        {{result:g1_v2_surface_footnoted_none_loss_rate:value}}         {{result:g1_v2_surface_footnoted_none_scored:value}}
    prose_labeled    (no surface)   {{result:g1_v2_surface_prose_labeled_none_loss_rate:value}}         {{result:g1_v2_surface_prose_labeled_none_scored:value}}

- The coded API tables — an estimate field and its margin-of-error field, side by side, exactly what the declared leg asks for — are where the consumer drops the uncertainty most. The handbook prose that scores nothing on the declared leg is where it survives.
- Compression drives loss harder than any surface property: family loss {{result:g1_v2_pooled_opus_E4_loss_rate_none:value}} with no budget, {{result:g1_v2_pooled_opus_E4_loss_rate_short:value}} at two sentences, {{result:g1_v2_pooled_opus_E4_loss_rate_tight:value}} at one sentence of thirty words. Pre-registered, and the outer intervals do not overlap.
- **Draft.** Measured on a pilot instrument frozen at v2; no product-level threshold is set, and the reviewer counts behind the failure classes are not yet calibrated against a human.

---

## Slide 15 — How the pieces relate: the knowledge architecture
...
## Slide 17 — Positioning against prior art
...
## Slide 18 — What's still missing (the honest slide)
...
## Slide 19 — January deliverable and pilot
