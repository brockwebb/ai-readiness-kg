...
**Source of truth, as of 2026-09-06 (DD-050):** the indicator tables in §2–§5d are a **rendered projection** of `framework/ai_readiness_framework.json`, which is the framework of record. Regenerate them with `python scripts/render_framework.py --write`; check them with `--check`, which fails on any cell that does not round-trip. The prose sections (§1, §1b, §6–§10) are authored markdown and are not generated. **Counts in the v0.2 paragraph above are stale**: the tables hold **47** indicator rows (48 nodes, G1 being two legs), not 45 — A10 and A11 were added in v0.2.1 and the header was never updated.
...
| A3 | Bulk access | Full-product bulk download exists and is linked from product page | AUTO | `w3c-dwbp-2017` (BP 17, provide bulk download: "enable consumers to retrieve the full dataset with a single request"); `wilkinson-2016-fair-guiding-principles` (A1 and A1.1: retrievable by their identifier using a standardized communications protocol that is open, free, and universally implementable); `dcat-us-1-1-schema` (distribution → downloadURL, "must be the **direct** download URL", distinguished there from accessURL, which is an indirect means of access; the Project Open Data metadata schema whose implementation guidance is OMB M-13-13) | `public` | draft |
