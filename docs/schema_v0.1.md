...
- 2026-08-27 (Desktop session, pending CC task `2026-08-27_chunked_pilot` ADDENDUM-01 §2 → will land as **v0.3.7**): §5's whole-document single-call protocol is **RETIRED** as an extraction unit (DD-023; `docs/research/kg_construction_methodology.md` §6.1/§6.5) — measured: 108–158K-token outputs, per-layer fallback doubling per-doc cost, and 65,637 settled tokens/chunk under the same contract chunked. Replacement, entering as v0.3.7 when its pilot is judged: chunk-local extraction (section-bounded ≤ 1,500-token units), anchor-based grounding (model emits shortest-unique anchor; harness locates and derives the span — locate-at-birth guarantee unchanged), salience-not-exhaustiveness for the Concept layer (§2's "exhaustive layer" wording is scoped to the retired contract), `diversion_reason` closed list enforced at parse, entity-type reconciliation at merge. Nothing in the type catalogue changes; this is the extraction protocol, not the schema of the graph.
- 2026-08-26 (task `2026-08-26_overnight_burn`, Lane 0 → **v0.3.4**): append-only. Instrument gains `grounding_spans` — a per-attribute map `{owner|year|method: verbatim span}` required by prompt v0.3.4 whenever the attribute is filled; the parser validates each entry (`grounding.is_grounded` against the document AND `grounding.covers(value)`) and **nulls the attribute at parse** (recorded as `nulled_at_parse` on the item) when the span is missing or non-covering — the node itself is not quarantined. Semantic edges (`has_component`/`subtype_of`/`consumes`/`extends`/`implements`) whose grounding span does not contain both endpoints' names route to `proposed_relationships` (never quarantine, never the graph). Cited-only instruments are extracted as Concepts with `mentions` (prompt rule). Implements the two `reextract_required` requirement blocks of `docs/research/2026-08-22_probe_decision.md`.
...
| Instrument | An existing assessment, survey, index, or benchmark. | name, owner, year, method, grounding_spans (per-attribute span map — v0.3.4; parser nulls uncovered attributes) |
