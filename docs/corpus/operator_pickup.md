...
Ordered by `t2_priority` (crosswalk demand, then T0 centrality). That ordering is **provisional — T0 coverage 29/178**; the demand component is coverage-independent and already final, the centrality component is not. `python -m kg.biblio resume` refreshes both.
...
| `webb-fcsm-nist-crosswalk` | fidelity: degraded | Docling could not convert this PDF; text came from pypdf, the converter DD-023 names as damaged. A clean or born-digital copy would restore layout fidelity. | https://doi.org/10.5281/zenodo.18772590 | 0 | 0 |  |
