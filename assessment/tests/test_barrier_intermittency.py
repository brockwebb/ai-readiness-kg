...
def test_all_attempts_clean_passes_and_reports_a_zero_refusal_fraction():
    score, evidence = NoBarriersProbe().evaluate_attempts([OK(), OK(), OK()], DIST)
...
def test_one_refusal_in_three_fails_and_carries_the_fraction():
    ...
    score, evidence = NoBarriersProbe().evaluate_attempts([OK(), REFUSED(), OK()], DIST)
...
def test_every_attempt_refused_is_also_fail_but_distinguishable_in_evidence():
    always = NoBarriersProbe().evaluate_attempts([REFUSED(), REFUSED(), REFUSED()], DIST)
    sometimes = NoBarriersProbe().evaluate_attempts([OK(), OK(), REFUSED()], DIST)
...
def test_per_attempt_statuses_are_recorded_in_order():
    score, evidence = NoBarriersProbe().evaluate_attempts(
...
def test_captcha_body_counts_as_a_refusal_even_at_status_200():
    ...
    score, evidence = NoBarriersProbe().evaluate_attempts([OK(), captcha], DIST)
    assert score == Score.FAIL
...
def test_transport_failure_across_attempts_fails_separately_from_refusal():
    ...
    score, evidence = NoBarriersProbe().evaluate_attempts([down, down], DIST)
    assert score == Score.FAIL
...
def test_no_attempts_is_reported_not_crashed():
    score, evidence = NoBarriersProbe().evaluate_attempts([], DIST)
    assert score == Score.FAIL
