...
CAPTIONS = {
    ...
    "cycle_over_cycle":
        ...
        "95 % interval. {rule_changed} No line, no delta, no arrow: a difference between two "
...
def caption(name: str, cfg: dict) -> str:
    """A figure's caption, with the cycle-dependent clause filled in.

    F5's marked legs used to be written into the caption as "A1 and A3": true for the cycle it
    was written for and false for the next one. The legs whose RULE changed are already
    declared in `figures.yaml` under the compare_to for the cycle being drawn — the same
    declaration the figure itself reads to draw the marks — so the caption reads it there
    rather than restating it, and a caption can no longer disagree with the chart above it.
    """
    body = CAPTIONS[name]
    if "{rule_changed}" not in body:
        return body
    cmp_ = cfg["compare_to"]
    changed = cmp_.get("rule_changed") or {}
    marked = (", ".join(f"{leg} ({how})" for leg, how in sorted(changed.items()))
              if changed else None)
    clause = (f"{marked} {'is' if len(changed) == 1 else 'are'} marked **rule changed: not "
              f"comparable** \u2014 a difference on those rows is the instrument moving, not "
              f"the host." if marked else
              "No leg's rule changed between these two, so every difference is the host.")
    if cmp_.get("note"):
        clause += (f" This pair is a RE-JUDGEMENT: {cmp_['note']} \u2014 both points rest on "
                   f"the SAME stored Observations, so every difference is the rule and none "
                   f"of it is the host.")
    return body.replace("{rule_changed}", clause)
    ...
    for name, f in figs.items():
        ...
        if fid:
            ...
            run(["seldon", "artifact", "update", fid, "--actor", "cc",
                 "-p", f"path={f['path']}", "-p", f"caption={caption(name, cfg)}",
        ...
        r = None if fid else run(["seldon", "artifact", "create", "Figure", "--actor", "cc",
                 ...
                 "-p", f"caption={caption(name, cfg)}",
                 "-p", f"data_source={', '.join(f['files']) or 'the Result registry only'}",
                 "-p", f"description={caption(name, cfg)} Generated by "
