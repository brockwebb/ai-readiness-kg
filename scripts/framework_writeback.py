...
COUNTS_BASIS = (
    ...
    "about assessment/harness/scan/rules, not about this file. actions counts REMEDIATES "
    "edges whose indicator is part of the framework and actions_on_candidate_indicators "
    "counts the rest, on the same DD-054 split: one Action per failing rule outcome, so the "
    "two together are the number of (indicator, outcome) pairs a prescription exists for. "
    "tools and preconditions count the requirements layer's AssessmentTool and Precondition "
    "nodes, which belong to no indicator and so have no candidate split; requires counts "
    "REQUIRES edges from the framework's indicators and requires_on_candidate_indicators the "
    "rest, on the same DD-054 split. "
    "Regenerated by scripts/framework_writeback.py::recount on every write-back."
)
...
def recount(g: dict) -> dict:
    ...
    return {
        ...
        # ---- the prescription layer (DN-005 §2.3), on the same DD-054 split ----
        # An Action is counted with the framework when the indicator it REMEDIATES is part of
        # the framework, and separately when that indicator is a candidate. The split is on the
        # edge's `to`, not on the Action itself, because a prescription inherits the standing
        # of the check it is bound to: A12's three actions are recorded and reported and are no
        # more adopted than A12 is.
        "actions": sum(1 for e in edges if e["type"] == "REMEDIATES" and e["to"] not in cand),
        "actions_on_candidate_indicators": sum(
            1 for e in edges if e["type"] == "REMEDIATES" and e["to"] in cand),
        # ---- the requirements layer (DN-005 §2.2 and §2.4) ----
        # A requirement node is shared by every indicator that needs it, so it has no
        # candidate standing of its own; the edge does, from the indicator at its tail.
        "tools": n_label("AssessmentTool", False),
        "preconditions": n_label("Precondition", False),
        "requires": sum(1 for e in edges if e["type"] == "REQUIRES" and e["from"] not in cand),
        "requires_on_candidate_indicators": sum(
            1 for e in edges if e["type"] == "REQUIRES" and e["from"] in cand),
