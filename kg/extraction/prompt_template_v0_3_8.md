<!--
v0.3.8 emission contract (chunked_pilot ADDENDUM-03 §3, Arm A2).
DERIVED FROM kg/extraction/prompt_template_v0_3_7.md BY ONE ADDITION AND NOTHING ELSE. The
v0.3.7 file is sha-pinned by the `v0_3_7` profile and is NOT edited; this is a new file with
its own pin, so Arm A's provenance is untouched and A2 has exactly one variable.

WHAT CHANGED: the FIRST GROUNDING RULE is restored. It is present in
kg/extraction/prompt_template.md (the whole-document v0.3.5 template) and absent from
kg/extraction/chunked_template.md, whose header nonetheless claims it carried over; v0.3.7
inherited that omission because it was derived from chunked_template.md (issue 53e2cf6e).

WHY: MEASURED on Arm A, 2026-08-29/30. 549 items were quarantined `span_partial`. All 549
derived spans were verbatim in the chunk and none was model-typed, so the anchor contract
itself held — what failed was that the model's OWN typed `name`/`claim_text` was not a
substring of the sentence the harness cut (382 not a substring at all, 167 differing only by
capitalization). The restored rule is the one that addresses that and only that.
...
normalize punctuation/spacing"), also absent from both chunked templates. Restoring two rules
at once would confound A2. This file changes ONE rule.

Everything else — the anchor contract, salience, the schema, the Instrument positive
criterion, the semantic-edge relation rule, evidence_grade, Measure.tier, the edge whitelist,
the closed diversion list and the emission order — is v0.3.7's text unchanged.
Rendering substitutes {{schema_version}}, {{document_id}}, {{chunk_id}}, {{document_text}}.
prompt_version: 0.3.8
-->
