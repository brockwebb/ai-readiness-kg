<!--
v0.3.9 emission contract (chunked_pilot ADDENDUM-04 §1, Arm A3).
DERIVED FROM kg/extraction/prompt_template_v0_3_8.md BY ONE ADDITION AND NOTHING ELSE. The
v0.3.8 file is sha-pinned by the `v0_3_8` profile and is NOT edited; this is a new file with
its own pin, so Arm A2's provenance is untouched and A3 again has exactly one variable.

WHAT CHANGED: the CHARACTER-EXACT rule is restored. It is present in
kg/extraction/prompt_template.md (the whole-document v0.3.5 template) and absent from
chunked_template.md, v0.3.7 and v0.3.8 (issue 53e2cf6e). ADDENDUM-03's A2 build left it out
deliberately so that A2 tested one rule; A3 adds it, so the three arms form a chain in which
each step moves one instruction.

ADAPTED TO THE ANCHOR CONTRACT. In the whole-document template this rule binds the
`grounding_span`, which under this contract the model does not emit at all — the harness cuts
it from the source. Here it binds the ANCHOR, which is the only text the model now copies.

WHY: MEASURED on Arm A2, 2026-08-30. `anchor_not_located` is A2's LARGEST quarantine class
(340 of 1,766 proposed items on the 44 shared chunks) and it rose against Arm A rather than
falling. Its subclasses: 167 not found in the chunk (49%), 153 non-unique (45%), 20 over the
10-token budget (6%). "Not found" means the model emitted an anchor that is not a substring of
the chunk — a transcription failure, which is exactly what this rule addresses.

WHAT IT CANNOT REACH: the non-unique subclass. Whether a correctly copied string occurs once
or twice is a property of the chunk text, not of the model's discipline, so no compliance
instruction touches it. It is pre-registered as expected residual.

Everything else — the anchor contract, the FIRST GROUNDING RULE restored in v0.3.8, salience,
the schema, the Instrument positive criterion, the semantic-edge relation rule, evidence_grade,
Measure.tier, the edge whitelist, the closed diversion list and the emission order — is
v0.3.8's text unchanged.
Rendering substitutes {{schema_version}}, {{document_id}}, {{chunk_id}}, {{document_text}}.
prompt_version: 0.3.9
-->
...
## FIRST GROUNDING RULE — name the entity the way the document names it

**Use the document's own surface form as the `name`.** The harness derives each item's span
from the source as the sentence your anchor sits in, and it then checks that the item's
...
An item whose derived span does not contain its own name is **rejected**. This is the single
most common way to lose an item you correctly anchored: the anchor lands on the right
sentence, and then a tidied-up or canonicalized name — `Data readiness` where the sentence
says `data readiness`, or `AI model accuracy` where the sentence says `inaccurate and
ineffective AI models` — throws the item away. A claim's `claim_text` is subject to the same
rule: state it in the document's words, not a summary of them.

This costs you nothing in output. It is a rule about WHICH words you copy, not how many.
